    ...
    class Decline < SlackRubyBotServer::Events::AppMentions::Mention
      ...
      user_in_channel_command 'decline' do |channel, challenger, data|
        ...
        if challenge
          ...
          channel.slack_client.say(channel: data.channel, text: "#{challenge.challenged.map(&:display_name).and} declined #{challenge.challengers.map(&:display_name).and} challenge.", gif: 'no')
          logger.info "DECLINE: #{channel} - #{challenge}"
        else
          channel.slack_client.say(channel: data.channel, text: 'No challenge to decline!')
