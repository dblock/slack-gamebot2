  ...
  module Commands
    class Promote < SlackRubyBotServer::Events::AppMentions::Mention
      ...
      user_in_channel_command 'promote' do |channel, user, data|
        ...
        if !users
          channel.slack_client.say(channel: data.channel, text: 'Try _promote @someone_.', gif: 'help')
          ...
          channel.slack_client.say(channel: data.channel, text: "#{captains.map(&:display_name).and} are already captains.")
          ...
          channel.slack_client.say(channel: data.channel, text: "#{captains.first.user_name} is already a captain.")
          ...
          users.each(&:promote!)
          channel.slack_client.say(channel: data.channel, text: "#{users.map(&:display_name).and} #{users.count == 1 ? 'has' : 'have'} been promoted to captain.", gif: 'power')
