...
describe SlackGamebot::Commands::ChallengeQuestion do
  ...
  it 'displays challenger elo first when stakes differ due to different tau values' do
    ...
    expect do
      expect(message: "<@bot_user_id> challenge? <@#{fresh_opponent.user_id}>", user: experienced_user, channel: channel).to respond_with_slack_message(
  ...
  context 'with against' do
    ...
    it 'displays elo at stake between two other players' do
      expect do
        expect(message: "<@bot_user_id> challenge? #{player1.slack_mention} against #{player2.slack_mention}", user: user, channel: channel).to respond_with_slack_message(
    ...
    it 'displays elo at stake between two other teams' do
      ...
      expect do
        expect(message: "<@bot_user_id> challenge? #{player1.slack_mention} #{player2.user_name} against #{player3.user_name} #{player4.user_name}", user: user, channel: channel).to respond_with_slack_message(
    ...
    it 'errors when no players on one side of against' do
      expect(message: "<@bot_user_id> challenge? against #{player2.slack_mention}", user: user, channel: channel).to respond_with_slack_message(
