...
describe SlackGamebot::Commands::Demote do
  ...
  context 'captain' do
    ...
    it 'demotes self' do
      ...
      expect(message: '<@bot_user_id> demote me', user: user.user_id, channel: channel).to respond_with_slack_message(
    ...
    it 'cannot demote another captain' do
      another_user = Fabricate(:user, channel: channel, captain: true)
      expect(message: "<@bot_user_id> demote #{another_user.user_name}", user: user.user_id, channel: channel).to respond_with_slack_message(
