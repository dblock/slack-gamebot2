  ...
  shared_examples_for 'leaderboard' do
    context 'with players' do
      ...
      it 'displays leaderboard sorted by elo' do
        expect(message: '<@bot_user_id> leaderboard', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_48}\n2. #{user_elo_42}"
      end

      it 'excludes unregistered users' do
        user_elo_48.unregister!
        expect(message: '<@bot_user_id> leaderboard', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_42}"
      end

      it 'limits to max' do
        expect(message: '<@bot_user_id> leaderboard 1', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_48}"
      end
      ...
      it 'supports infinity' do
        ...
        expect(message: '<@bot_user_id> leaderboard infinity', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_48}\n2. #{user_elo_44}\n3. #{user_elo_43}\n4. #{user_elo_42}"
      end

      it 'supports -infinity' do
        user_elo_43 = Fabricate(:user, channel: channel, elo: 43, wins: 2, losses: 3)
        user_elo_44 = Fabricate(:user, channel: channel, elo: 44, wins: 2, losses: 3)
        expect(message: '<@bot_user_id> leaderboard -infinity', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_42}\n2. #{user_elo_43}\n3. #{user_elo_44}\n4. #{user_elo_48}"
      end

      it 'supports -number' do
        user_elo_43 = Fabricate(:user, channel: channel, elo: 43, wins: 2, losses: 3)
        expect(message: '<@bot_user_id> leaderboard -2', channel: channel, user: user).to respond_with_slack_message "1. #{user_elo_42}\n2. #{user_elo_43}"
      end
    end

    context 'without players' do
      it 'says no players' do
        expect(message: '<@bot_user_id> leaderboard', channel: channel, user: user).to respond_with_slack_message "There're no ranked players."
      end
    end
  end
