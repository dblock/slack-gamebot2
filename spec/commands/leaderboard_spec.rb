  ...
  shared_examples_for 'leaderboard' do
    context 'with players' do
      ...
      it 'excludes unregistered users' do
        ...
        expect(message: '@gamebot leaderboard').to respond_with_slack_message "1. #{user_elo_42}"
      end
    ...
    context 'without players' do
      it 'says no players' do
        expect(message: '@gamebot leaderboard').to respond_with_slack_message "There're no ranked players."
      end
    end
  end
