  ...
  shared_examples_for 'matches' do
    ...
    context 'with many matches' do
      ...
      it 'displays top 10 matches' do
        ...
        expect(message: '@gamebot matches', user: user.user_id, channel: match1.challenge.channel).to respond_with_slack_message([
          "#{match1} 3 times",
          "#{match0} once"
        ].join("\n"))
      end
      ...
      it 'limits number of matches' do
        expect(message: '@gamebot matches 1', user: user.user_id, channel: match1.challenge.channel).to respond_with_slack_message([
          "#{match1} 3 times"
        ].join("\n"))
      end

      it 'displays only matches for requested users' do
        expect(message: "@gamebot matches #{match1.challenge.challenged.first.user_name}", user: user.user_id, channel: match1.challenge.channel).to respond_with_slack_message(
          "#{match1} 3 times"
        )
      end
      ...
      it 'displays only matches for requested users with a limit' do
        ...
        expect(message: "@gamebot matches #{match1.challenge.challenged.first.user_name} 1", user: user.user_id, channel: match1.challenge.channel).to respond_with_slack_message(
          "#{match1} 3 times"
        )
      end
    end
    ...
    context 'with a doubles match' do
      ...
      it 'displays user matches' do
        expect(message: '@gamebot matches', user: user.user_id, channel: match.challenge.channel).to respond_with_slack_message(
          "#{match} once"
        )
      end
    end
    ...
    context 'without matches' do
      it 'displays' do
        expect(message: '@gamebot matches', user: user.user_id, channel: 'channel').to respond_with_slack_message('No matches.')
      end
    end
    ...
    context 'matches in prior seasons' do
      ...
      it 'displays user matches in current season only' do
        expect(message: '@gamebot matches', user: user.user_id, channel: match2.challenge.channel).to respond_with_slack_message(
          "#{match2} once"
        )
      end
    end
