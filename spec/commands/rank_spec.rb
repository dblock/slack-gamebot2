...
describe SlackGamebot::Commands::Rank do
  ...
  shared_examples_for 'rank' do
    ...
    it 'ranks the requester if no argument is passed' do
      expect(message: '<@bot_user_id> rank', user: user_elo_42.user_id, channel: channel).to respond_with_slack_message "3. #{user_elo_42.user_name}: 3 wins, 2 losses (elo: 42)"
    end

    it 'ranks someone who is not ranked' do
      user = Fabricate(:user, channel: channel)
      expect(message: '<@bot_user_id> rank', user: user, channel: channel).to respond_with_slack_message "#{user.user_name}: not ranked"
    end

    it 'ranks someone who is not ranked by default' do
      user1 = Fabricate(:user, channel: channel)
      Fabricate(:user, channel: channel)
      expect(message: '<@bot_user_id> rank', user: user1, channel: channel).to respond_with_slack_message "#{user1.user_name}: not ranked"
    end

    it 'ranks someone who is not ranked by name' do
      user1 = Fabricate(:user, channel: channel)
      Fabricate(:user, channel: channel)
      expect(message: "<@bot_user_id> rank #{user1.user_name}", user: user1, channel: channel).to respond_with_slack_message "#{user1.user_name}: not ranked"
    end

    it 'ranks someone else who is not ranked by name' do
      user1 = Fabricate(:user, channel: channel)
      Fabricate(:user, channel: channel)
      expect(message: "<@bot_user_id> rank #{user1.user_name}", user: user_elo_42, channel: channel).to respond_with_slack_message "#{user1.user_name}: not ranked"
    end

    it 'ranks one player by slack mention' do
      expect(message: "<@bot_user_id> rank #{user_elo_42.slack_mention}", user: user, channel: channel).to respond_with_slack_message "3. #{user_elo_42}"
    end

    it 'ranks one player by user_id' do
      expect(message: "<@bot_user_id> rank <@#{user_elo_42.user_id}>", user: user, channel: channel).to respond_with_slack_message "3. #{user_elo_42}"
    end

    it 'shows the smallest range of ranks for a list of players' do
      users = [user_elo_38, user_elo_67].map(&:slack_mention)
      expect(message: "<@bot_user_id> rank #{users.join(' ')}", user: user, channel: channel).to respond_with_slack_message "2. #{user_elo_67}\n3. #{user_elo_42}\n4. #{user_elo_38}"
    end
  end
