...
describe SlackGamebot::Commands::Reset do
  ...
  it 'requires a team name' do
    expect(User).not_to receive(:reset_all!).with(channel)
    expect(message: '@gamebot reset', user: captain).to respond_with_slack_message("Missing channel, confirm with _reset #{channel.slack_mention}_.")
  end

  it 'requires a matching channel name' do
    expect(User).not_to receive(:reset_all!).with(channel)
    expect(message: '@gamebot reset invalid', user: captain).to respond_with_slack_message("Invalid channel, confirm with _reset #{channel.slack_mention}_.")
  end
  ...
  it 'resets with the correct channel mention' do
    ...
    expect(message: "@gamebot reset #{channel.slack_mention}", user: captain).to respond_with_slack_message('Welcome to the new season!')
  end

  it 'resets with the correct channel id' do
    Fabricate(:match, channel: channel)
    expect(User).to receive(:reset_all!).with(channel).once
    expect(message: "@gamebot reset #{channel.channel_id}", user: captain).to respond_with_slack_message('Welcome to the new season!')
  end
  ...
  it 'cannot be reset unless any games have been played' do
    expect(message: "@gamebot reset #{channel.slack_mention}", user: captain).to respond_with_slack_message('No matches have been recorded.')
  end
