...
describe SlackGamebot::Commands::Reset, vcr: { cassette_name: 'user_info' } do
  ...
  it 'requires a captain' do
    ...
    expect(message: '@gamebot reset').to respond_with_slack_message("You're not a captain, sorry.")
  end

  it 'requires a team name' do
    expect(User).not_to receive(:reset_all!).with(team)
    expect(message: '@gamebot reset').to respond_with_slack_message("Missing team name or id, confirm with _reset #{team.name}_ or _reset #{team.team_id}_.")
  end

  it 'requires a matching team name' do
    expect(User).not_to receive(:reset_all!).with(team)
    expect(message: '@gamebot reset invalid').to respond_with_slack_message("Invalid team name or id, confirm with _reset #{team.name}_ or _reset #{team.team_id}_.")
  end

  it 'resets with the correct team name' do
    Fabricate(:match)
    expect(User).to receive(:reset_all!).with(team).once
    expect(message: "@gamebot reset #{team.name}").to respond_with_slack_message('Welcome to the new season!')
  end

  it 'resets with the correct team id' do
    Fabricate(:match)
    expect(User).to receive(:reset_all!).with(team).once
    expect(message: "@gamebot reset #{team.team_id}").to respond_with_slack_message('Welcome to the new season!')
  end
  ...
  it 'cannot be reset unless any games have been played' do
    expect(message: "@gamebot reset #{team.name}").to respond_with_slack_message('No matches have been recorded.')
  end
