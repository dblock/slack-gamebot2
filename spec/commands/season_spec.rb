...
describe SlackGamebot::Commands::Season do
  ...
  shared_examples_for 'season' do
    context 'no seasons' do
      it 'seasons' do
        expect(message: '<@bot_user_id> season', user: user, channel: channel).to respond_with_slack_message "There're no seasons."
      end
    end
    ...
    context 'current season' do
      context 'with a match' do
        ...
        it 'returns current season' do
          ...
          expect(message: '<@bot_user_id> season', user: user, channel: channel).to respond_with_slack_message current_season.to_s
        end
        ...
        context 'after reset' do
          ...
          it 'returns current season' do
            expect(message: '<@bot_user_id> season', user: user, channel: channel).to respond_with_slack_message 'No matches have been recorded.'
          end
        end
      end
