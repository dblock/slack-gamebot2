...
describe SlackGamebot::Commands::Sucks do
  ...
  it 'sucks' do
    expect(message: '<@bot_user_id> sucks', user: user, channel: channel).to respond_with_slack_message(
      "No #{user.slack_mention}, you suck!"
    )
  end

  it 'suck' do
    expect(message: '<@bot_user_id> you suck', user: user, channel: channel).to respond_with_slack_message(
      "No #{user.slack_mention}, you suck!"
    )
  end
  ...
  it 'does not conflict with a player name that contains suck' do
    expect(message: '<@bot_user_id> challenge suckarov', user: user, channel: channel).to respond_with_slack_message(
