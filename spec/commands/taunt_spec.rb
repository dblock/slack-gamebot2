...
describe SlackGamebot::Commands::Taunt do
  ...
  it 'taunts one person by user id' do
    ...
    expect(message: "<@bot_user_id> taunt <@#{victim.user_id}>", user: user, channel: channel).to respond_with_slack_message(
  ...
  it 'taunts one person by user name' do
    victim = Fabricate(:user, channel: channel)
    expect(message: "<@bot_user_id> taunt #{victim.user_name}", user: user, channel: channel).to respond_with_slack_message(
      "#{user.user_name} says that #{victim.user_name} sucks at this game!"
    )
  end
  ...
  it 'taunts multiple users by user id' do
    ...
    expect(message: "<@bot_user_id> taunt <@#{victim1.user_id}> <@#{victim2.user_id}> <@#{victim3.user_id}>", user: user, channel: channel).to respond_with_slack_message(
  ...
  it 'taunts multiple users by user name' do
    ...
    victim2 = Fabricate(:user, channel: channel)
    victim3 = Fabricate(:user, channel: channel)
    expect(message: "<@bot_user_id> taunt #{victim1.user_name} #{victim2.user_name} #{victim3.user_name}", user: user, channel: channel).to respond_with_slack_message(
      "#{user.user_name} says that #{victim1.user_name}, #{victim2.user_name} and #{victim3.user_name} suck at this game!"
    )
  end

  it 'no entered user to taunt' do
    expect(message: '<@bot_user_id> taunt', user: user, channel: channel).to respond_with_slack_message(
