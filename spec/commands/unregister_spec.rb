...
describe SlackGamebot::Commands::Unregister do
  ...
  context 'with another team' do
    ...
    it 'cannot unregister an unknown user by name' do
      expect(message: "@gamebot unregister #{user2.user_name}", user: captain).to respond_with_slack_message("I don't know who #{user2.user_name} is!")
    end
  end

  it 'unregisters self' do
    expect(message: '@gamebot unregister', user: user).to respond_with_slack_message("I've removed #{user.slack_mention} from the leaderboard.")
    expect(user.reload.registered).to be false
  end

  it 'unregisters self via me' do
    expect(message: '@gamebot unregister me', user: user).to respond_with_slack_message("I've removed #{user.slack_mention} from the leaderboard.")
    expect(user.reload.registered).to be false
  end

  it 'unregisters another user' do
    expect(message: "@gamebot unregister #{another_user.user_name}", user: captain).to respond_with_slack_message("I've removed #{another_user.slack_mention} from the leaderboard.")
  ...
  it 'unregisters multiple users' do
    ...
    expect(message: "@gamebot unregister #{user1.user_name} <@#{user2.user_id}>", user: captain).to respond_with_slack_message(
