...
describe SlackGamebot::Commands::Unregister do
  ...
  it 'requires a captain to unregister someone' do
    expect(message: '<@bot_user_id> unregister someone', channel: channel, user: user).to respond_with_slack_message("You're not a captain, sorry.")
  end
  ...
  it 'registers, then unregisters a previously unknown user' do
    ...
    expect do
      expect(message: '<@bot_user_id> unregister', channel: channel, user: 'user1').to respond_with_slack_message("I've removed <@user1> from the leaderboard.")
    end.to change(User, :count).by(1)
    expect(User.where(channel: channel, user_id: 'user1').first.registered).to be false
  end

  it 'unregisters self' do
    expect(message: '<@bot_user_id> unregister', user: user, channel: channel).to respond_with_slack_message("I've removed #{user.slack_mention} from the leaderboard.")
    expect(user.reload.registered).to be false
  end

  it 'unregisters self via me' do
    expect(message: '<@bot_user_id> unregister me', user: user, channel: channel).to respond_with_slack_message("I've removed #{user.slack_mention} from the leaderboard.")
    expect(user.reload.registered).to be false
  end

  it 'unregisters another user' do
    expect(message: "<@bot_user_id> unregister #{another_user.user_name}", user: captain, channel: channel).to respond_with_slack_message("I've removed #{another_user.slack_mention} from the leaderboard.")
  ...
  it 'unregisters multiple users' do
    ...
    expect(message: "<@bot_user_id> unregister #{user1.user_name} <@#{user2.user_id}>", user: captain, channel: channel).to respond_with_slack_message(
  ...
  context 'with another team' do
    ...
    it 'cannot unregister an unknown user by name' do
      expect(message: "<@bot_user_id> unregister #{user2.user_name}", user: captain, channel: channel).to respond_with_slack_message("I don't know who #{user2.user_name} is!")
    end
  end
end
